...
public class GeoPlusCodeHandler implements GeoHashTypeProvider {
     ...
     * that can fit within a long value, so we could used 14.
     * Yet, somewhere ES looses precision for lat/lng while aggregating, so keep it safe.
     */
    public static final int[] ALLOWED_LENGTHS = {4, 6, 8, 10, 11, 12};

    /**
     * Maximum plus code length (without the '+' symbol) that we support.
     */
    private static final int MAX_LENGTH = ALLOWED_LENGTHS[ALLOWED_LENGTHS.length - 1];
